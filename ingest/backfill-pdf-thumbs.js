#!/usr/bin/env node
/**
 * Backfill thumbnails for PDF documents that are missing them.
 */
...
async function main () {
  ...
  console.log('[backfill] %d PDF documents need thumbnails', rows.length)
  ...
  for (const row of rows) {
    ...
    try {
      const success = await generateThumbnail(row.file_path, thumbDest, 'pdf')
      if (success) {
        db.db.prepare('UPDATE documents SET thumb_path = ? WHERE id = ?').run(thumbDest, row.id)
        ok++
      } else {
        console.warn('[backfill] No output for %s (%s)', row.id, row.file_name)
        fail++
      }
    } catch (err) {
